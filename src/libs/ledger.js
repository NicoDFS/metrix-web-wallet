import metrixJsLib from 'metrixjs-lib'
...
export default class Ledger {
  ...
  static async generateTx(keyPair, ledger, path, from, to, amount, fee, utxoList, rawTxFetchFunc = () => {}) {
    ...
    const inputs = []
    const paths = []
    const selectUtxo = metrixJsLib.utils.selectTxs(utxoList, amount, fee)
    const rawTxCache = {}
    ...
    const outputs = new metrixJsLib.TransactionBuilder(keyPair.network)
  ...
  static async generateSendToContractTx(keyPair, ledger, path, from, contractAddress, encodedData, gasLimit, gasPrice, fee, utxoList, rawTxFetchFunc = () => {}) {
    ...
    const outputs = new metrixJsLib.TransactionBuilder(keyPair.network)
    const contract =  metrixJsLib.script.compile([
      OPS.OP_4,
