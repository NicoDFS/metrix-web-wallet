<template>
  <v-card>
    ...
    <v-card-text>
      <v-alert color="metrix-main" value="true">{{ $t('safe_send.info') }}</v-alert>
      <v-stepper non-linear vertical v-model="step">
        <v-stepper-step color="metrix-main" step="1" editable>Generate Base Info (At online computer)</v-stepper-step>
        <v-stepper-content step="1">
          <template v-if="mode === 'offline'">
            <v-alert color="metrix-main" value="true">{{ $t('safe_send.info1_offline') }}</v-alert>
            <v-btn color="success" @click.native="step = 2">{{ $t('common.next') }}</v-btn>
          </template>
          <template v-else>
            <v-alert color="metrix-main" value="true">{{ $t('safe_send.info1_online') }}</v-alert>
            <v-text-field
        ...
        <v-stepper-step color="metrix-main" step="2" editable>Generate Tx (At offline computer)</v-stepper-step>
        <v-stepper-content step="2">
          <template v-if="mode === 'offline'">
            <v-alert color="metrix-main" value="true">{{ $t('safe_send.info2_offline') }}</v-alert>
          ...
          <template v-else>
            <v-alert color="metrix-main" value="true">{{ $t('safe_send.info2_online') }}</v-alert>
            <v-btn color="success" @click.native="step = 3">{{ $t('common.next') }}</v-btn>
          </template>
        </v-stepper-content>

        <v-stepper-step color="metrix-main" step="3" editable>Broadcast Tx (At online computer)</v-stepper-step>
        <v-stepper-content step="3">
          <template v-if="mode === 'offline'">
            <v-alert color="metrix-main" value="true">{{ $t('safe_send.info3_offline') }}</v-alert>
          </template>
          <template v-else>
            <v-alert color="metrix-main" value="true">{{ $t('safe_send.info3_online') }}</v-alert>
            <template v-if="!fileParsed">
              <file-reader @upload="handleFile" color="info"></file-reader>
            </template>
            <template v-if="fileParsed">
      ...
      <v-card>
        <v-card-title>
          <span class="headline">
            ...
            <v-chip label>{{this.amount}}MRX</v-chip>
            {{ $t('send.to_address') }}
