    ...
    methods: {
      ...
      async confirmAddress() {
        ...
        try {
          ...
          } else if (mrc20.checkSymbol(this.symbol)) {
            if (wallet.extend.ledger) {
              this.rawTx = 'Please confirm tx on your ledger...'
            }
            const token = mrc20.getTokenBySymbol(this.symbol)
            const encodedData = mrc20.encodeSendData(token, this.address, this.amount)
      ...
      async searchAddToken() {
        ...
        try {
          const tokenInfo = await mrc20.fetchTokenInfo(this.addTokenAddress)
          ...
          if ((e.response && e.response.status === 404) || (e.message === 'this contract is not a mrc20 token')) {
      ...
      confirmAddToken() {
        mrc20.addCustomToken(this.addTokenAddress, this.addTokenName, this.addTokenSymbol, this.addTokenDecimals)
        this.initTokens()
      ...
      initTokens() {
        const tokenList = [{ text: 'MRX', value: 'MRX' }]
        mrc20.getTokenList().forEach((token) => {
